...
namespace ConsoleAppFileManager
{
    /// <summary>
    /// Основной класс программы, реализующий файловы менеджер.
    /// </summary>
    class Program
    {
        /// <summary>
        ...
        private static string ReadDriveName(string[] driveNames)
        {
            try
            {
                ...
                do
                {
                    Console.Write("Введите название диска, на который хотите перейти (вместе с :\\ или :/) >>>");
                ...
                foreach (DriveInfo currentDrive in availableDrives)
                {
                    ...
                    if (currentDrive.TotalSize > 0)
                    {
                        ...
                        Console.WriteLine("[" + new string('|', 20 - (int)(partOfUsedSapce * 20)) +
                            new string(' ', (int)(partOfUsedSapce * 20)) + $"]свободно {currentDrive.TotalFreeSpace}" +
                            $"/{currentDrive.TotalSize} байт ({(partOfUsedSapce * 100).ToString("F2")}%).");
                    }
                    else
                    {
        ...
        /// <summary>
        /// Основной метод программы, запрашивает команды у пользователя и реагирует на них.
        /// </summary>
